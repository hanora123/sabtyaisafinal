...
المسارات تكون تحت:
- `/v1`
- المنفذ الافتراضي (Port): `3000` (أو `4000` حسب الإعدادات في `.env`).

## 5) تشغيل الواجهة (Frontend)
من داخل `frontend`:
- `npm run dev`
- المنفذ الافتراضي: `3000` (Next.js).
- تأكد من ضبط `NEXT_PUBLIC_API_URL` في ملف `.env.local` ليشير إلى رابط الـ Backend.

## 6) تهيئة أولية (مطلوب مرة واحدة)
1) إنشاء Admin (مرة واحدة فقط إذا كانت قاعدة البيانات فارغة):
- `POST /v1/auth/seed-default-admin`
- يمكن استخدام ملف `backend/setup.js` لأتمتة هذه الخطوة.
...
## 7) إدخال المخزون يدويًا (لأن النظام ورقي حاليًا)
